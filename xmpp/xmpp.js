            ...
            case "message":
                ...
                switch (msg.root.attributes.type) {
                    ...
                    case "groupchat":
                        ...
                        if (body.trim().toLowerCase().startsWith("/redeem ")) {
                            let codeToRedeem = body.trim().substring(8).trim();
                            handleRedeemCode(ws, accountId, displayName, codeToRedeem, roomName, jid, resource);
                            return;
                        }
...
function RemoveClient(ws, joinedMUCs) {
    ...
    // Safely parse lastPresenceUpdate.status
    let ClientStatus = {};
    try {
        if (client && client.lastPresenceUpdate && client.lastPresenceUpdate.status && typeof client.lastPresenceUpdate.status === "string" && client.lastPresenceUpdate.status.trim().length > 0) {
            ClientStatus = JSON.parse(client.lastPresenceUpdate.status);
        }
    } catch (err) {
        ClientStatus = {};
    }

    updatePresenceForFriends(ws, "{}", false, true).catch(() => {});
...
async function getPresenceFromFriends(ws, accountId, jid) {
    let friends = await Friends.findOne({ accountId: accountId }).lean().catch(() => null);
    if (!friends || !friends.list) return;
    ...
    accepted.forEach(friend => {
        ...
        const lastPresence = ClientData.lastPresenceUpdate || { away: false, status: "{}" };
        const statusString = typeof lastPresence.status === "string" ? lastPresence.status : JSON.stringify(lastPresence.status || {});

        if (lastPresence.away) xml = xml.element("show", "away").up().element("status", statusString).up();
...
async function updatePresenceForFriends(ws, body, away, offline) {
    ...
    if (SenderIndex == -1 || !SenderData) return;

    // Ensure lastPresenceUpdate exists
    global.Clients[SenderIndex].lastPresenceUpdate = global.Clients[SenderIndex].lastPresenceUpdate || { away: false, status: "{}" };
    global.Clients[SenderIndex].lastPresenceUpdate.away = away;
    global.Clients[SenderIndex].lastPresenceUpdate.status = typeof body === "string" ? body : JSON.stringify(body);

    let friends;
    try {
        friends = await Friends.findOne({ accountId: SenderData.accountId });
    } catch {
        friends = null;
    }
    ...
    accepted.forEach(friend => {
        ...
        if (away) xml = xml.element("show", "away").up().element("status", global.Clients[SenderIndex].lastPresenceUpdate.status).up();
        else xml = xml.element("status", global.Clients[SenderIndex].lastPresenceUpdate.status).up();

        try {
            ClientData.client.send(xml.toString());
        } catch (err) {
            // swallow send errors per original behavior
        }
    });
}
...
async function handleRedeemCode(ws, accountId, displayName, code, roomName, jid, resource) {
    try {
        if (!code || code.length === 0) {
            sendSystemMessage(ws, jid, "Please provide a code to redeem. Usage: /redeem CODE", roomName, displayName, accountId, resource);
            return;
        }

        const codeLower = code.toLowerCase().trim();
        const redeemCode = await RedeemCodes.findOne({ code_lower: codeLower });

        if (!redeemCode) {
            sendSystemMessage(ws, jid, "Invalid code. Please check the code and try again.", roomName, displayName, accountId, resource);
            return;
        }

        if (redeemCode.used) {
            sendSystemMessage(ws, jid, "This code has already been redeemed.", roomName, displayName, accountId, resource);
            return;
        }

        if (redeemCode.expiresAt && new Date() > redeemCode.expiresAt) {
            sendSystemMessage(ws, jid, "This code has expired.", roomName, displayName, accountId, resource);
            return;
        }
        ...
        const updatedProfile = await Profiles.findOneAndUpdate(
            filter,
            { $inc: { 'profiles.common_core.items.Currency:MtxPurchased.quantity': redeemCode.vbucks } },
            { new: true }
        );

        if (!updatedProfile) {
            sendSystemMessage(ws, jid, "Error: Profile not found. Please contact support.", roomName, displayName, accountId, resource);
            return;
        }

        await Profiles.updateOne(filter, { $inc: { 'profiles.profile0.items.Currency:MtxPurchased.quantity': redeemCode.vbucks } });
        ...
        const newQuantityProfile0 = profile0.items['Currency:MtxPurchased'].quantity + redeemCode.vbucks;
        ...
        const lootList = [{
            ...
            "quantity": redeemCode.vbucks
        }];

        common_core.items[purchaseId] = {
            "templateId": `GiftBox:GB_MakeGood`,
            "attributes": {
                "fromAccountId": `[Code Redemption]`,
                "lootList": lootList,
                "params": {
                    "userMessage": `Code Redeemed: ${redeemCode.code}`
                },
        ...
        sendSystemMessage(ws, jid, `Successfully redeemed code! You received ${redeemCode.vbucks} V-Bucks!`, roomName, displayName, accountId, resource);
        log.xmpp(`Player ${displayName} (${accountId}) redeemed code ${redeemCode.code} for ${redeemCode.vbucks} V-Bucks`);

    } catch (err) {
        log.error("Error redeeming code:", err);
        sendSystemMessage(ws, jid, "An error occurred while redeeming the code. Please try again later.", roomName, displayName, accountId, resource);
    }
}

function sendSystemMessage(ws, jid, message, roomName, displayName, accountId, resource) {
    // Send message back to the player who redeemed
    try {
        ws.send(XMLBuilder.create("message")
            .attribute("to", jid)
            .attribute("from", getMUCmember(roomName, "System", "SYSTEM", "system"))
            .attribute("xmlns", "jabber:client")
            .attribute("type", "groupchat")
            .element("body", message).up().toString());
    } catch {}
}
